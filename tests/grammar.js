...
function id(x) { return x[0]; }
 const {S, VP, NP, PN, V, PRO, DET, N, AND} = require("./ast.js"); var grammar = {
    ...
    {"name": "S", "symbols": ["NP", "VP", {"literal":"."}], "postprocess": ([np, vp]) => S(np, vp)},
    {"name": "VP", "symbols": ["V", "_", "NP"], "postprocess": ([v, _, np]) => VP(V(v[0]), np)},
    {"name": "NP", "symbols": ["PN", "_"], "postprocess": ([pn]) => NP(PN(pn[0]))},
    {"name": "NP", "symbols": ["PRO", "_"], "postprocess": ([pro]) => NP(PRO(pro[0]))},
    {"name": "NP", "symbols": ["DET", "_", "N", "_"], "postprocess": ([det, _, n]) => NP(DET(det[0]), N(n[0]))},
    {"name": "NP$string$1", "symbols": [{"literal":"a"}, {"literal":"n"}, {"literal":"d"}], "postprocess": function joiner(d) {return d.join('');}},
    {"name": "NP", "symbols": ["NP", "NP$string$1", "_", "NP"], "postprocess": ([np1, sp1, sp2, np2]) => NP(AND(np1, np2))},
